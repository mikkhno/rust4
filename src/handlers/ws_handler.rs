...
pub(crate) async fn handle_connection(ws: WebSocket, users: Users) {
    ...
    // Loop to continuously listen for incoming messages from the user
...
async fn broadcast_message(message: Message, users: &Users) {
    ...
    // Attempt to convert the incoming message into a text string
    if let Ok(text) = message.to_str() {
        ...
        // Remove disconnected clients from the users list
